...
list*list_merge(list*l1,list*l2){
	if(!l1||!l2)EPRET(EINVAL);
	list*ll1=list_last(l1),*ll2=list_first(l2);
	if(!ll1||!ll2||list_add(ll1,ll2)!=0)return NULL;
	return l1;
}
...
int list_free_all_reverse(list*point,runnable_t*datafree){
	errno=0;
	if(!point)ERET(EINVAL);
	list*cur=list_last(point),*prev;
	if(!cur)cur=point;
	do{
		prev=cur->prev;
		list_free_item(cur,datafree);
