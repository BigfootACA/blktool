...
static int get_terminal_dimension(int *cols, int *lines){
	int c = 0, l = 0;
	...
	if(GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE),&con)){
		c = con.dwSize.X;
		l = con.dwSize.Y;
	}
#endif
...
int scols_table_set_default_symbols(struct libscols_table *tb)
{
	...
	if (!scols_table_is_ascii(tb) && is_utf8) {
		/* tree chart */
